...
not add public ingress rules for ports 3000, 5000, 8000, 8080, 9000, or 9001.
...
<http://localhost:5000>, the inference API documentation at
<http://localhost:8000/docs>, Grafana at <http://localhost:3000>, and the MinIO
console at <http://localhost:9001>. DVC and other S3-compatible clients use the
MinIO API at <http://localhost:9000>. Credentials are stored in the server's
`.env` file.
